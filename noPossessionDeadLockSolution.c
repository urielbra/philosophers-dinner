...
void * philosopher (void *num)
{
	...
	while (f = food_on_table ()) {
		...
		if(pthread_mutex_trylock(&chopstick[right_chopstick]) != 0 ) { // TREAT DEADLOCK
			printf("Philosopher %ld cannot get right chopstick, he will drop the left one\n", id);
			pthread_mutex_unlock (&chopstick[left_chopstick]);
			// sleep (rand()%3); // Wait for other philosephers try to pick
		} else {
